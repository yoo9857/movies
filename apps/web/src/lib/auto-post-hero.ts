...
export type PostPhotoSourceType = "PERSON" | "MOVIE";
...
export async function autoAttachPostHero(postId: string) {
  ...
  const queries: { query: string; alt: string; sourceType: PostPhotoSourceType }[] = [
    ...post.people.map(({ person }) => ({ query: person.name, alt: person.name, sourceType: "PERSON" as const })),
  ...
  for (const subject of queries) {
    ...
    try {
      ...
      return {
        ...
        foundFor: subject.query,
        photoTitle: photo.title,
        captureDay: photo.day,
        candidateWidth: photo.width,
        candidateHeight: photo.height,
      };
    } catch {
